...
public class WPCaseJDBCDAO2 implements WPCaseDAO_Interface{
	...
	private static final String GET_ALL = 
			"SELECT * FROM WED_PHOTO_CASE where wed_photo_status= 0 order by WED_PHOTO_CASE_NO";
	private static final String GET_ALL_OFF = 
			"SELECT * FROM WED_PHOTO_CASE where wed_photo_status= 1 order by WED_PHOTO_CASE_NO";
	...
	public List<WPCaseVO> getAlloff() {		
		List<WPCaseVO> list = new ArrayList<WPCaseVO>();		
		Connection con = null;
		Statement stmt = null;
		ResultSet rs = null;
		...
		try {
			...
			con = DriverManager.getConnection(url, userid, passwd);
			stmt = con.createStatement();
			rs = stmt.executeQuery(GET_ALL_OFF);
	...
	public static void main(String args[]) {
		WPCaseJDBCDAO2 dao = new WPCaseJDBCDAO2();
		WPCaseVO vo = new WPCaseVO();
		//新增
		vo.setVender_id("V030");
		vo.setWed_photo_name("測試標題");
		vo.setWed_photo_intro("測試內文");
		vo.setWed_photo_price(37000);
		
		java.sql.Timestamp wed_photo_addtime = new Timestamp(System.currentTimeMillis());		
		vo.setWed_photo_addtime(wed_photo_addtime);
		vo.setWed_photo_status(0);
		dao.insert(vo);
		
		//查全部(上架)
//		List<WPCaseVO> list = dao.getAllon();
//		for (WPCaseVO casevo : list) {
...
//		dao.delete("WPC052");
		...
		//查全部(下架)
//		List<WPCaseVO> list = dao.getAlloff();
//		for (WPCaseVO casevo : list) {
//			System.out.println(casevo.getWed_photo_case_no()+" "+casevo.getWed_photo_name());						
//		}
		
		//查一筆方案
//		WPCaseVO voc =  dao.getOneWPno("WPC001");
//		System.out.println(voc.getWed_photo_case_no());
//		System.out.println(voc.getWed_photo_name());
		
		//修改
//		vo.setWed_photo_case_no("WPC016");
//		vo.setWed_photo_name("修改標題測試");
//		vo.setWed_photo_intro("修改內文測試");
//		vo.setWed_photo_price(66666);
//		vo.setWed_photo_status(0);
//		dao.updateWPCase(vo);
	}
}
