...
<script src="<%=request.getContextPath() %>/js/prod/myOrders.js" type="text/javascript" charset="utf-8"></script>
<style>
.myBg {
...
<div class="container mt-2"><h2>買方訂單管理</h2></div>
    ...
    <div class="collapse navbar-collapse" id="main_nav2">
      <ul class="navbar-nav w-100 d-flex justify-content-around" id="myNav">
        <li class="nav-item flex-fill">
          <a class="nav-link text-center" style="width:100%" href="<%=request.getContextPath()%>/order/order.do?action=getDifferentStatusOrders_buyer&order_status=All">全部</a> 
<!--           <hr color="blue"> -->
        </li>
        
        <li class="nav-item flex-fill">
         <a class="nav-link text-center" href="<%=request.getContextPath()%>/order/order.do?action=getDifferentStatusOrders_buyer&order_status=1">待出貨</a> 
        </li>
        <li class="nav-item flex-fill">
          <a class="nav-link text-center" href="<%=request.getContextPath()%>/order/order.do?action=getDifferentStatusOrders_buyer&order_status=2">待收貨</a>
        </li>
        <li class="nav-item flex-fill">
          <a class="nav-link text-center" href="<%=request.getContextPath()%>/order/order.do?action=getDifferentStatusOrders_buyer&order_status=3">完成</a>
        </li>
        <li class="nav-item flex-fill">
          <a class="nav-link text-center" href="<%=request.getContextPath()%>/order/order.do?action=getDifferentStatusOrders_buyer&order_status=0">取消</a>
        </li>
      </ul>
    </div> <!-- collapse .// -->
  </div> <!-- container .// -->
    ...
    <div class="card-header d-flex justify-content-start myParent myBg align-items-center" id="headingOne_${myCount.count }" data-toggle="collapse" data-target="#collapseOne_${myCount.count }" aria-expanded="true" aria-controls="collapseOne_${myCount.count }">
    	<span style="display:inline" class="ml-3">
 		...
 		<span style="display:none" class="ml-3">
          ...
          <button class='btn btn-light myBtn' data-toggle="modal" data-target="#orderModal" data-order_no="${ordervo.order_no }" data-myTarget="${membreSvc.getSeller(ordervo.order_no).getMembre_id()}" 
          >取消訂單</button>
          </c:if>
           <c:if test='${ordervo.order_status == "2"}'>
           <button class='btn btn-light myBtn' data-toggle="modal" data-target="#orderModal" 
           data-order_no="${ordervo.order_no }" 
           data-myTarget="${membreSvc.getSeller(ordervo.order_no).getMembre_id()}"
            >確認取貨</button>
           </c:if>
            <c:if test='${ordervo.order_status == "3"}'>
            <form action="<%=request.getContextPath()%>/order/shopping.do" method="post">
            <input type="hidden" name="action" value="buyAgain">
            <input type="hidden" name="order_no" value="${ordervo.order_no }">
           <button class='btn btn-light buyAgain mt-2'>再買一次</button>
           </form>
           </c:if>
    </div>
